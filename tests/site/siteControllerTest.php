...
class adaloginControllerTest extends PHPUnit_Framework_TestCase {
	...
	public function test_loginform()	{
		...
		$this->assertEquals(
			// without :8080 -> 'https://adatom.hu/ada/v1/oauth2/auth?response_type=code&client_id=APP_ID_COMES_HERE&redirect_uri=https%3A%2F%2Flocalhost%2Fcomponents%2Fcom_adalogin%2Findex.php%3Fredi%3DaHR0cDovL2xvY2FsaG9zdC8%3D',
            'https://adatom.hu/ada/v1/oauth2/auth?response_type=code&client_id=APP_ID_COMES_HERE&redirect_uri=https%3A%2F%2Flocalhost%3A8080%2Fcomponents%2Fcom_adalogin%2Findex.php%3Fredi%3DaHR0cDovL2xvY2FsaG9zdDo4MDgwLw%3D%3D',
	...
	public function test_doadalogout_loggedin() {
		global $testData,$componentName;

		$this->setupConfig();
		$controller = new AdaloginController();
		if(JFactory::getUser()->guest) { // Check if the user is already logged in. If not:
			$testData->addDbResult(JSON_decode('{"id":1,"username":"testElek"}'));
			$testData->setInput('adaid', 123);
			$testData->setInput('adaemail', 'test@test.hu');
			$testData->setInput('assurance', '["magyar"]');
			$testData->setInput('redi', base64_encode('http://localhost/redi.php'));
